...
class _$ResultTearOff {
  ...
  Failure<T> failure<T>({required ApiExceptions error}) {
    return Failure<T>(
      error: error,
    );
  }
...
abstract class Failure<T> implements Result<T> {
  const factory Failure({required ApiExceptions error}) = _$Failure<T>;

  ApiExceptions get error => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $FailureCopyWith<T, Failure<T>> get copyWith =>
      throw _privateConstructorUsedError;
}
